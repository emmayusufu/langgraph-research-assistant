...
export function AIPanel({
  ...
  const handlePreset = (key: PresetKey) => {
    ...
    if (key === "outline") {
      runRequest({ action: "outline", topic: context.slice(0, 200) || "document", context });
      return;
    }
    ...
    >
      ...
      {ai.status === "idle" && view === "root" && (
        <>
          ...
          <PromptInput
            placeholder={
              mode === "selection" ? "Tell AI what to do with this…" : "Ask AI to write anything…"
            }
      ...
      {ai.status === "generating" && <StreamingPreview text={ai.draft} stage="writing" />}
