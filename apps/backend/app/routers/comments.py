...
async def create_thread(doc_id: uuid.UUID, body: CreateThreadRequest, user: User = Depends(current_user)):
    role = await docs_db.get_role(doc_id, user.id)
    await authorize(role, "write")
    result = await db.create_thread(body.thread_id, doc_id, user.id, body.body)
    return {"thread_id": str(body.thread_id), "message_id": str(result["id"]), "created_at": result["created_at"].isoformat()}
...
async def set_resolved(thread_id: uuid.UUID, body: ResolveRequest, user: User = Depends(current_user)):
    await _authz_thread_write(thread_id, user)
